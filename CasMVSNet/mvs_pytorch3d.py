...
parser.add_argument('--batch_size', type=int, default=1, help='testing batch size')#7 GPUs can use maximum 18 batches +- 2
...
parser.add_argument('--num_view', type=int, default=3, help='num of view')#2 is blur, 10 is too much
...
def save_scene_depth(model):
    # dataset, dataloader  
    mesh = pytorch3d.io.load_objs_as_meshes(files = ["rd.obj"], device= device)
    
    RR = [] 
    KK = []
    TT = []
    
    R = np.load('/root/lhs/pytorch3d_test/pytorch3d_R00.npy')
    K = np.load('/root/lhs/pytorch3d_test/pytorch3d_K00.npy')
    T = np.load('/root/lhs/pytorch3d_test/pytorch3d_T00.npy')
    R1 = np.load('/root/lhs/pytorch3d_test/pytorch3d_R01.npy')
    K1 = np.load('/root/lhs/pytorch3d_test/pytorch3d_K01.npy')
    T1 = np.load('/root/lhs/pytorch3d_test/pytorch3d_T01.npy')
    R2 = np.load('/root/lhs/pytorch3d_test/pytorch3d_R02.npy')
    K2 = np.load('/root/lhs/pytorch3d_test/pytorch3d_K02.npy')
    T2 = np.load('/root/lhs/pytorch3d_test/pytorch3d_T02.npy')
    R3 = np.load('/root/lhs/pytorch3d_test/pytorch3d_R03.npy')
    K3 = np.load('/root/lhs/pytorch3d_test/pytorch3d_K03.npy')
    T3 = np.load('/root/lhs/pytorch3d_test/pytorch3d_T03.npy')
    RR.append(R)
    RR.append(R1)
    RR.append(R2)
    RR.append(R3)
    ...
    cameras = pytorch3d.utils.cameras_from_opencv_projection(R = torch.tensor([R,R1,R2,R3]),
                                                            tvec = torch.tensor([T,T1,T2,T3]),
                                                            camera_matrix = torch.tensor([K,K1,K2,K3]),image_size = torch.tensor([[2048, 2048]])).to(device)
    ...
    imgs = renderer(mesh.extend(4))

    # print(images[0].size())
    np_arr = np.array(255*imgs[0].to('cpu'), dtype=np.uint8)
    img = Image.fromarray(np_arr)

    #img.show()
    img.save('./test.png')
    # fig = plot_batch_individually([mesh,cameras])
    # #fig.show()
    # fig.write_image("ha.png")
    ...
    for i in range(4):
        ...
        proj_mat[0, :3, :3] = RR[i]@np.array([[-1,0,0],[0,-1,0],[0,0,1]])
        proj_mat[0, :3, 3] = TT[i]
        proj_mat[1, :3, :3] = KK[i]
        ...
        if i == 0:  # reference view
            ...
            ####################### fix this code later 
            depth_values = np.arange(depth_min, args.interval_scale * (args.numdepth - 0.5) + depth_min, args.interval_scale,
                                        dtype=np.float32)

        #all
        imgs = np.stack(np.array(imgs.to('cpu'), dtype=np.float32)).transpose([0, 3, 1, 2])
    ...
    with torch.no_grad():
        ...
        outputs = model(images, sample_cuda["proj_matrices"], sample_cuda["depth_values"])
        end_time = time.time()
        print('only inference: ' +str(end_time-start_time))
        outputs = tensor2numpy(outputs)

        del sample_cuda
        filenames = "./pytorch3d_Test"
